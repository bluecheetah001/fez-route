...
use petgraph::visit::{
    Dfs, EdgeFiltered, EdgeRef, IntoEdgeReferences, IntoNodeIdentifiers, IntoNodeReferences,
};
...
pub fn optimize(graph: &Graph<Node, Edge>, required_bits: i32) {
    ...
    let renderer = Renderer::new("rendered").unwrap();

    problem
        .optimize_mip(|reason| match reason {
            Reason::AddLazyExprs(problem) => {
                let value_graph = value_graph(graph, &problem, edges);
                if let Some(expr) =
            ...
            // TODO heuristic is topo order assuming it is a valid path: has edges, required bits, keys, ect
            // TODO branch up where heuristic is violated: edge leading out of topo (into a loop), 
        })
        .unwrap();
...
fn no_2_cycles(graph: &Graph<Node, Edge>, edges: VarRefs) -> Vec<Expr> {
    graph
        .edge_references()
...
fn no_3_cycles(graph: &Graph<Node, Edge>, edges: VarRefs) -> Vec<Expr> {
    graph
        .node_references()
        .flat_map(|(i, n)| {
            let sources = graph
